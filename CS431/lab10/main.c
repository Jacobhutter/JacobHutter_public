...
double Kp_x = 1.2, Kd_x = 0.55, Ki_x = 0.00;
double Kp_y = 1.2, Kd_y = 0.55, Ki_y = 0.00;
...
void __attribute__((interrupt, auto_psv)) _T1Interrupt(void) {
  ...
  if (select == X_DIM) {
    ...
    int i =0;
    for(i=0; i< 5; i++)
        samples[i] = SampleADC_X();
    sort_me(samples);
    Xpos = (uint16_t)samples[2];
    ...
    int i =0;
    for(i=0; i< 5; i++)
        samples[i] = SampleADC_Y();
    sort_me(samples);
    Ypos = (uint16_t)samples[2];
