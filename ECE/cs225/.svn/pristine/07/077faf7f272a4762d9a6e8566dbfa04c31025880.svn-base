...
class BinaryTree
{
  ...
  private:
    ...
    Node * mirror(Node * leaf); // recursive helper function

    bool isOrdered(const Node * leaf, T upper, T lower) const;
    void printPaths(const Node * leaf, vector<T> vect) const;
    int sumDistances(const Node * leaf, int level) const;
