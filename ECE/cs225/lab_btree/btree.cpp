...
{  /////
     ...
     */
     V element = find(subroot->children[first_larger_idx], key);
     if (subroot->is_leaf && element != key)
      return V();
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
     ...
     * | 1 |   | 3 | 6 | 8 |
     *  */
      //parent->children.push_back(&BtreeNode());
    /*
     ...
     *
     */
     //parent->children.back() = parent->children[child_idx]->children[order/2];
    ...
    parent->children.insert(parent->children.begin()  +  child_idx + 1,new_right); // insert new child
    parent->elements.insert(parent->elements.begin()  +  child_idx,child->elements[mid_elem_idx]); // insert new element

    for(mid_elem_itr = mid_elem_itr + 1; mid_elem_itr  != child->elements.end(); mid_elem_itr++ ){ // plus one because element has already been removed

          new_right->elements.push_back(*mid_elem_itr);
    }
    for(mid_child_itr = child->children.begin() + mid_child_idx ; mid_child_itr != child->children.end(); mid_child_itr++){
          new_right->children.push_back(*mid_child_itr);
    }


    size_t size = child->elements.size();
    for(size_t i = mid_elem_idx ; i < size; i++){
      child->elements.pop_back();
    }

    size = child->children.size();
    for(size_t i = size; i >  mid_child_idx; i--){
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    if(subroot->is_leaf && find(subroot, pair.key) == V()){
      subroot->elements.emplace(first_larger_idx + subroot->elements.begin(), pair); // insert new element
      //BTreeNode * node = new BTreeNode(subroot->is_leaf,order);
      BTreeNode * node = NULL;
      subroot->children.emplace(subroot->children.begin() + first_larger_idx, node); // insert new child
      if(subroot->children.size() == 1){
          //BTreeNode * node2 = new BTreeNode(subroot->is_leaf,order);
          BTreeNode * node2 = NULL;
          subroot->children.emplace(subroot->children.begin() + first_larger_idx + 1, node2); // insert new child
      }
    }
