...
vector<vector<string>> AnagramDict::get_all_anagrams() const
{
    ...
    { auto srch = std::find(available.begin(),available.end(),word.first);

      //auto lookup = available.find(word->first); // if word is available
      if(srch != available.end()){
          words = get_anagrams(word.first); // words is the anagrams
          for(auto & eraser : words){
            available.erase(std::find(available.begin(),available.end(),eraser)); // erase every word in the available word category
          }
      if(words.size() > 1 )
