...
//this code, written by jacob hutter, will accept an image and apply a filter cosine, convolution, flip, grayscale, or color threshold. 
// the main function calls individual functions here and they are executed using 1-d arrays to represent the 2-d images
// each pixel value containing some red green blue and alpha numbers are dealt with accordingly to present the desired result image
...
{ 
    double C = 0;
     ...
     for(x=-radius;x<=radius;x++){
		...
		C = fabs(C);                      //ensuring c will always be positive 0-255
		location = (y+radius)*(2*radius+1) + (x+radius);
		cos_filter[location] = C;
	}
 }
		for(i=0;i<((2*radius+1)*(2*radius+1));i++){
			sum += cos_filter[i];	                  // adding together all filter vals 
 }
		for(i=0;i<((2*radius+1)*(2*radius+1));i++){
...
double size1 = (2*radius +1)*(2*radius+1);
    if( radius > 0 ){
			 ...
			 for(x=0;x<width;x++){
			  ...
			  for(i=-radius;i<=radius;i++){       // sum2 calculations
			   ...
			   for(j=-radius;j<=radius;j++){     // sum1 calculations 
				   if((((y+j)*(width)+(x+i)) <= size) && (((y+j)*(width)+(x+i)) >=0) && (((j+radius)*(2*radius+1)+(i+radius)) >= 0) && (((j+radius)*(2*radius+1)+(i+radius)) <=size1)&&(x+i >=0) &&(x+i<width)&&(y+j>=0)&&(y+j<height)){
					...
					sum1B += (filter[((j+radius)*(2*radius+1)+(i+radius))] * in_blue[((y+j)*(width)+(x+i))]);	    // this section will set the leftmost sum to itself plus the incrementable parts
				}
	 ...
	 if(sum1R > 255)
		 sum1R  = 255;
	 if(sum1G > 255)       // this part checks whether or not the result is within a reasonable range 
		 sum1G  = 255;
	 if(sum1B > 255)
		 sum1B  = 255;
	 if(sum1R < 0)
	 ...
	 if(sum1B < 0)
		 sum1B = 0;
   if(((y*width + x) >= 0) && ((y*width +x) <= (width *height))){        //checks final bounds and submits output to sum2 
		out_red[(y*width + x)] = sum1R;
		out_green[(y*width + x)] = sum1G;
		out_blue[(y*width + x)] = sum1B;  
 }          
		sum1R = 0;
		//sum2R = 0;
		sum1G = 0;
	  ...
	  for(i=0;i<(width*height);i++){
		 out_red[i] = in_red[i];               // if radius is negative then set input to output
		 out_green[i] = in_green[i];
		 out_blue[i] = in_blue[i];
	 }
	}
	for(i=0;i<(width*height);i++)
		out_alpha[i]=in_alpha[i];      //alpha will always be passed to itself 
// printf("%i\n",in_red[0]);
// printf("%i\n",out_red[0]);
...
{ 
		...
		double sum = 0;
		for(i=0;i<(width*height);i++){
		sum = (in_red[i] * gmonomult[0]) + (in_green[i] * gmonomult[1]) + (in_blue[i] * gmonomult[2]);  //simply sums the products and applies to output
		out_red[i]= sum;
		out_green[i]= sum;
		out_blue[i]= sum;
                 ...
                 uint8_t *out_blue,uint8_t *out_alpha,int width,int height)
{
	...
	for(row = 0; row < height ; row ++){
		for(column =0; column < width;column++){
			if((((height -1)-row)*width +column >= 0) && (((height -1)-row)*width +column) <= (height*width)){     // row (h-1)-row of input is now row h of output
                 ...
                 int red_threshold,int blue_threshold,int green_threshold)
{
     int i;
		...
		if( (in_red[i] <= red_threshold) || (in_green[i] <= green_threshold) || (in_blue[i] <= blue_threshold)){     //any color value below a specified threshold is now black
			out_red[i] = 0;
			out_green[i] = 0;
			out_blue[i] = 0;
			out_alpha[i] = 255;
