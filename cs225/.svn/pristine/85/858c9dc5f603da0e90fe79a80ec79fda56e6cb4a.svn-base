...
{  bool flag = false;
    ...
    string word4;
    word3 = word1;
    std::transform(word3.begin(), word3.end(), word3.begin(), ::toupper); // convert to uppercase to match string
    word4 = word2;
    std::transform(word4.begin(), word4.end(), word4.begin(), ::toupper); // convert to uppercase to match string
    auto lookup3 = dict.find(word3); // find the upper case
    if(lookup3 == dict.end()){
      return false;
    }
    auto lookup4 = dict.find(word4);
    if(lookup4 == dict.end())
      return false;
    if(lookup3->second == lookup4->second)
      flag = true;
    /* Your code goes here! */
    return flag;
