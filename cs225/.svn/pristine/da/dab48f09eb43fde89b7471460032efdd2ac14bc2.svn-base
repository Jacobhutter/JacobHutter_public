...
void Quadtree::pruneSize(QuadtreeNode * subtree , int tolerance, int & total) const{
    ...
    if(allchildren(subtree,subtree,tolerance)){
      total++;
      return;
    }
...
void Quadtree::rec_count(QuadtreeNode * subtree,  int  & count) const{
  count++;
  if(isLeafNode(subtree))
    return;

  rec_count(subtree->nwChild,count);
  rec_count(subtree->neChild,count);
  rec_count(subtree->swChild,count);
  rec_count(subtree->seChild,count);
...
if(numLeaves <= 0)
  return 0;
   //calculate maximum possible distance between two pixels
    //(255 - 0 )^2 + 255^2 + 255^2 = 3std::pow(255,2);
  int dist = 3*std::pow(255,2);
  //int count = 0;
  //rec_count(root,count);
  //if(numLeaves > count)
    //return dist;
  dist /= 2; // divide for binary search
  return binary_search(numLeaves,dist,dist);
}
int Quadtree::binary_search(int numLeaves, int dist, int step) const{
    if(step <= 0){
      while(pruneSize(dist) > numLeaves)
        dist++;
    ...
    int guess = pruneSize(dist); // calculate numLeaves for given tolerance
    if(guess > numLeaves)
      return binary_search(numLeaves,dist + step/2,step/2); // if not save the step size as step over 2 and recurse with +/-
    else if(guess < numLeaves)
      return binary_search(numLeaves,dist - step/2,step/2);
    else{
      int i;
      for(i = 0; i < dist; i++){
          if(pruneSize(dist - i) != numLeaves)
            break;
      }
