                                    ...
                                    const string& word_list_fname)
{
    ...
    if(wordsFile.is_open()){  // get every five letter word
      while(getline(wordsFile,current_word)){ // read into current word;
    ...
    for(string five_word : five){
      string missing_first = five_word.substr(1); // miss the first char;
      char first = five_word[0]; // store initial character;
      string missing_second = first + five_word.substr(2); // get word with second char missing
