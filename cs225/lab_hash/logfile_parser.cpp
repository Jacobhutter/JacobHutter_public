...
LogfileParser::LogfileParser(const string& fname) : whenVisitedTable(256)
{
    ...
    while (infile.good()) {
         ...
         */
          bool found = false;
          string entry = ll.customer+"/"+ll.url;
          if(whenVisitedTable.keyExists(entry)){
            if(whenVisitedTable.find(entry) < ll.date)
              whenVisitedTable[entry] = ll.date;
          }
          else
            whenVisitedTable[entry] = ll.date;
          for(auto it = uniqueURLs.begin(); it != uniqueURLs.end(); it++){
              if(*it == ll.url)
                found = true;
            }
            if(found== false)
...
bool LogfileParser::hasVisited(const string& customer, const string& url) const
{
    ...
    //SCHashTable<string, bool> customerlist(256);
