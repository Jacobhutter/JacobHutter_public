                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    if(singleQueue.front()->freq.getFrequency() <= mergeQueue.front()->freq.getFrequency()){  //tiebreaker rule
      smallest = singleQueue.front();
      singleQueue.pop();
      return smallest;
    }
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
     for(size_t i =0; i< frequencies.size(); i++){
       TreeNode * addition = new TreeNode(frequencies[i]);
       singleQueue.push(addition);
     }
     TreeNode *temp1 = NULL;
     TreeNode *temp2 = NULL;
     while((!singleQueue.empty() || mergeQueue.size() != 1) && (!mergeQueue.empty() || singleQueue.size() != 1 )){
       temp1 = removeSmallest(singleQueue,mergeQueue); // find smallest element
       temp2 = removeSmallest(singleQueue,mergeQueue); // find second smallest element
       TreeNode * swag = new TreeNode(temp1->freq.getFrequency() + temp2->freq.getFrequency());
       swag->left = temp1;
       swag->right = temp2;
       mergeQueue.push(swag);
     }
     if(!singleQueue.empty()){
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
     ...
     */
     ...
     if(bfile.getNextBit()){
       ...
       TreeNode * new_node = new TreeNode(f);
       new_node->left = NULL;
       new_node->right = NULL;
       return new_node;
     }
     else{
       TreeNode * notleaf = new TreeNode(0);
       notleaf->left = readTree(bfile);
       notleaf->right = readTree(bfile);
       return notleaf;

     }
    //return new_node; // replaceme!
}
