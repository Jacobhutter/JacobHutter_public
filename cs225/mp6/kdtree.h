...
class KDTree
{
  ...
  private:
    ...
    void findNearestNeighbor(const Point<Dim>& query, Point<Dim> * &local_best, size_t lower, size_t upper, int curDim) const;
    ...
    void rec_KDTree(int left, int right, int dimension);

    int partition(int left, int right, int pivotIndex, int dimension); // partition subfunction for quickselect

    void select(int left, int right, int n, int dimension);
