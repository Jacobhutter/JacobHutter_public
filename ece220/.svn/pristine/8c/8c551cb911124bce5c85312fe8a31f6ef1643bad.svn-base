...
/* this code written by Jacob Hutter will adequetly optimize space and abide by an optimization postfix expression array. it initializes a slicing tree
and then provides functionality such as rotate, swap topology and etc. */
...
int is_internal_node(node_t* ptr) {
  // TODO: (remember to modify the return value appropriately)
   if(ptr->cutline != UNDEFINED_CUTLINE) // undefined cutline means it is a leaf node
   	return 1;
...
int is_in_subtree(node_t* a, node_t* b) {
	...
	if( a == b )
		return 1;
    if(is_in_subtree(a->left,b))   // recurse to call left
    	return 1;
    if(is_in_subtree(a->right,b)) // recurse to call right
    	return 1;
...
void rotate(node_t* ptr) {
		...
		height = temp;
		ptr->module->w = width;
		ptr->module->h = height; // use classic 3 var swap method to reverse the two vals
		return;
...
void recut(node_t* ptr) {
  ...
  assert(ptr->module == NULL && ptr->cutline != UNDEFINED_CUTLINE);
	...
	if(ptr->cutline == 0){  // recuts the program to the desired V or H determined by current 
		ptr->cutline = H;
		return;
}
	else 
...
// Swap the two modules between two given leaf nodes in the slicing tree. // swap the leafs associated or the module of the node given?
void swap_module(node_t* a, node_t* b) {
  ...
  assert(b->module != NULL && b->cutline == UNDEFINED_CUTLINE); //if undefined cutline, then the modules are numbers
	module_t * temp = NULL;
	temp = a->module;
	a->module = b->module;    // simply swaps module 
	b->module = temp;
	return;
...
void swap_topology(node_t* a, node_t* b) {
  ...
  assert(a->parent != NULL && b->parent != NULL);
	...
	temp1 = a;
	temp2 = b;
	node_t * previousa = a->parent;
	node_t * previousb = b->parent;
	if(previousa->left == a){
	previousa->left = b;
	b->parent = previousa;
	if(previousb->left == b){
		previousb->left = temp1;
		previousb->left->parent = previousb;
}
	else {
		previousb->right = temp1;
		previousb->right->parent = previousb;   // swaps all topologies of the given nodes
}
}
	else{
	previousa->right = b;
	previousa->right->parent = previousa;
	if(previousb->left == b){
		previousb->left = temp1;
		previousb->left->parent = previousb;
}
	  ...
	  previousb->right = temp1;
		previousb->right->parent = previousb;
 }
}
 return;
}
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
  ...
  if(ptr->module!=NULL){ 
  	expression[*nth].cutline = UNDEFINED_CUTLINE;       // goes through and verifies the postfix traversal that the modules are supposed to have 
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  if( n == (num_modules -1 )){ // come to last node in the tree
  	node_t * lastl = (node_t *)malloc(sizeof(node_t));
  	lastl->right = NULL;
  	lastl->left = NULL;
  	lastl->cutline = UNDEFINED_CUTLINE;
  	lastl->parent = par;
  	lastl->module = &(modules[n]);
  	return lastl; 
  } 
  node_t * root = (node_t *)malloc(sizeof(node_t));           // starts off with a root that has a null parent and builds one right node and then steps into the left one recursively
  ...
  root->parent = par;
  ++n;
  root->left = init_slicing_tree(root,n);
