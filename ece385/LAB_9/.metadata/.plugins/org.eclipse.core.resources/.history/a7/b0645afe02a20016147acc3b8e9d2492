...
#define to_hw_port (volatile uint32_t*) 0x00000090 // actual address here
#define to_hw_sig (volatile char*) 0x00000080 // actual address here
#define to_sw_port (volatile uint32_t*) 0x00000070 // actual address here
  ...
  uint8_t state[16];
	...
	*to_hw_port = 0;
    ...
    while(loop < 10){
      ...
      ShiftRows(state);
      MixColumns(state);
      AddRoundKey(state, keyschedule, loop);
      loop++;
    }
    loop = 10;
    SubBytes(state);
    ...
    printf("\n");
		...
		for (i = 0; i <= 12; i = i + 4)
		{	*to_hw_sig = 1;
			*to_hw_port = word(state[i],state[i+1],state[i+2],state[i+3]); // encryptedMsg is your encrypted message
			//printf("%x\n",*to_hw_port);
		...
		*to_hw_sig = 2; // go to read key state


		for (i = 0; i <=12; i = i + 4)
		{
			*to_hw_sig = 1; // go to read key state
			*to_hw_port = word(key[i],key[i+1],key[i+2],key[i+3]);
			// Consider to use charToHex() if your encrypted message is a string.
			while (*to_sw_sig != 1);
			*to_hw_sig = 2;
			while (*to_sw_sig != 0);
