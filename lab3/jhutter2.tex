...
\\ The system is not BIBO stable, if we have a bounded input $z = -e^{\frac{+-i8\pi}{10}}$ then the output approaches infinity so the system is not BIBO stable. The system is stable when $|z| > e^{\frac{i8\pi}{10}}$ and unstable when $|z| \leq e^{\frac{i8\pi}{10}}$.
\end{figure}
