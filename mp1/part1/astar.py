...
while(bfs):
	...
	if(x > 0):
		if(maze[x - 1][y] == 0):
			...
			bfs.append(((x - 1, y), int(maze[x - 1][y]) + abs(x - 1 - end[0][0]) + abs(y - end[0][1])))
			nodes_added += 1
		elif(maze[x - 1][y] > 0):
			if((int(maze[x][y]) + 1) < int(maze[x - 1][y])):
				maze[x - 1][y] = int(maze[x][y]) + 1.1
				bfs.append(((x - 1, y), int(maze[x - 1][y]) + abs(x - 1 - end[0][0]) + abs(y - end[0][1])))
				nodes_added += 1
		elif(maze[x - 1][y] == -5):
	...
	if(y > 0):
		if(maze[x][y - 1] == 0):
                        ...
                        bfs.append(((x, y - 1), int(maze[x][y - 1]) + abs(x - end[0][0]) + abs(y - 1 - end[0][1])))
			nodes_added += 1
		elif(maze[x][y - 1] > 0):
			if((int(maze[x][y]) + 1) < int(maze[x][y - 1])):
				maze[x][y - 1] = int(maze[x][y]) + 1.3
				bfs.append(((x, y - 1), int(maze[x][y - 1]) + abs(x - end[0][0]) + abs(y - 1 - end[0][1])))
				nodes_added += 1
                elif(maze[x][y - 1] == -5):
